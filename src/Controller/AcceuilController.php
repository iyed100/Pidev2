...
public function index(
    ...
    $hotels = $hotelRepo->findAllWithCoworkingSpaces();

    // Formatage simple pour la vue
    $offresCompletes = [];
    
    foreach ($hotels as $hotel) {
        // Pour chaque hôtel, trouve un transport aléatoire
        $transports = $transportRepo->findBy([], null, 1);
        $transport = count($transports) > 0 ? $transports[0] : null;
        
        $offresCompletes[] = [
            'hotel' => $hotel,
            'coworking' => $hotel->getCoworkingSpaces()->first() ?: null,
            'transport' => $transport
        ];
    }
